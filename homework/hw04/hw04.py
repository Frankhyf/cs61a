...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    if is_planet(m):
        return True #如果是planet那直接就是balanced
     #如果不是的话就按下面来，先通过end函数得到左右arm下面的mobile   
    l,r = end(left(m),end(right(m)))
    return balanced(l) and balanced(r) and #满足条件2
    total_weight(l)*length(left(m)==total_weight(r)*lenth(right(m)))#满足条件1
