...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for son in branches(t):
            if berry_finder(son):#判断每个branch是否有berry，上一个if的内容
                return True
        return False
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
      return tree(label(t),[tree(x) for x in leaves])#leaves是传入的参数值
    else:
      return tree(label(t),[sprout_leaves(branch,leaves)for branch in 
      branches(t)])#每次都要重新判断一下分支是否是树叶
