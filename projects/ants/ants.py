...
class ThrowerAnt(Ant):
    ...
    min_range = 0
    max_range = float('inf')
    def nearest_bee(self, beehive):#为保证继承的灵活性，可以把range设置为类属性而不是函数里面的参数，
                                   #以便要重新添加新的和range有关的蚂蚁类型时，只需要继承throwerAnt
                                   #类并更改min和max即可
        ...
        copy = self.place
        cur = 0
        while cur <self.min_range and copy != beehive:
                 copy = copy.entrance
                 cur+=1
        while cur <=self.max_range and copy != beehive:
                 if copy.bees==[]:
                     copy = copy.entrance
                     cur+=1
                 else:
                     return random.choice(copy.bees)
        return None
        # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount>=self.armor:
            ...
            to_damage = amount
        for bees in self.place.bees[:]:#切片拷贝，是一种浅拷贝，会复制列表中元素的引用但不会
                                       #创建新的独立对象,但拷贝本身和原列表不是一个对象
            Insect.reduce_armor(bees,to_damage)
        #    def remove_from(self, place):
        ...
        # END Problem 5
        #核心问题：死了之后place变成none,那么如何指代原来的place
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.is_fake_queen == 0:
            self.real_queen = 1
            QueenAnt.is_fake_queen = 1
        else:
            self.real_queen = 0
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.real_queen ==0 :
            ...
            return 
        Former = self.place.exit
        while Former != None :
            if Former.ant != None and Former.ant.not_buffed:
                Former.ant.damage = 2*Former.ant.damage
                Former.ant.not_buffed = False
            Former = Former.exit
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        for bees in self.place.bees[:]:
            bees.reduce_armor(self.damage)#思考为什么不用传入bees这个参数
        # END Problem Optional 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        result = {}
        distance = 0
        while next is not  beehive:
              if next.bees != []:
                  for bees in next.bees:
                      result[bees]=distance#新增字典的写法
              if next.ant and next.ant is not self:
