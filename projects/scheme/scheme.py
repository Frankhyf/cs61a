...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    """
    # Evaluate atoms
    if scheme_symbolp(expr):#如果是某个符号表示值
        return env.lookup(expr)# lookup是找到环境中符号（symbol)对应的值
    elif self_evaluating(expr):#如果是123456这种数那就直接返回自己
        return expr

    # All non-atomic expressions are lists (combinations)
    if not scheme_listp(expr):#如果不是List那说明出错了                                                                                
        raise SchemeError('malformed list: {0}'.format(repl_str(expr)))
    first, rest = expr.first, expr.rest
    if scheme_symbolp(first) and first in SPECIAL_FORMS:#如果第一个是符号并且是SPECIAL_FORMS
        return SPECIAL_FORMS[first](rest, env)
    else:#如果不是special form那就说明是+ - * /
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operator = scheme_eval(first,env)
        validate_procedure(operator) #为什么在这里出现
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        value = scheme_eval(expressions.rest.first,env) #expressions.rest.rest = nil
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            #return scheme_eval(clause.rest.first,env) 注意scheme_eval的特性
            if clause.rest == nil:
                return test
            else:
